...
// Session-stored pending deep link info
export type PendingRecord =
  | { action: 'connect'; createdAt: number; origin: string }
  | { action: 'signMessage'; createdAt: number; origin: string; message: string }
  | { action: 'signAndSubmit'; createdAt: number; origin: string; payload: EntryFunctionPayload };

// Data returned by Petra deep link callback (base64-encoded JSON in `data` param)
export interface PetraCallbackData {
...
export function readPending(state: string): PendingRecord | null {
  const s = sessionStorage.getItem(PENDING_PREFIX + state);
  if (!s) return null;
  try { return JSON.parse(s) as PendingRecord; } catch {
    return null;
  }
}
...
export function handlePetraCallback(): void {
  try {
    ...
    const finish = (res: DeepLinkResult, redirectToOrigin = true) => {
    ...
    const data: PetraCallbackData = dataB64 ? base64DecodeUtf8ToObj<PetraCallbackData>(dataB64) : {};
    ...
    if (action === 'connect') {
      ...
      try {
        ...
        finish({ ok: false, state, action, error: 'Failed to derive shared key: ' + msg });
        return;
      }
